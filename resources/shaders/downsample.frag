...
void main()
{
    ...
    vec3 a = texture(u_Input, vec2(xn2, yp2)).rgb;
    vec3 b = texture(u_Input, vec2(TexCoords.x,       yp2)).rgb;
    vec3 c = texture(u_Input, vec2(xp2, yp2)).rgb;
    
    vec3 d = texture(u_Input, vec2(xn2, TexCoords.y)).rgb;
    vec3 e = texture(u_Input, vec2(TexCoords.x,       TexCoords.y)).rgb;
    vec3 f = texture(u_Input, vec2(xp2, TexCoords.y)).rgb;

    vec3 g = texture(u_Input, vec2(xn2, yn2)).rgb;
    vec3 h = texture(u_Input, vec2(TexCoords.x,       yn2)).rgb;
    vec3 i = texture(u_Input, vec2(xp2, yn2)).rgb;

    vec3 j = texture(u_Input, vec2(TexCoords.x - x, TexCoords.y + y)).rgb;
    vec3 k = texture(u_Input, vec2(TexCoords.x + x, TexCoords.y + y)).rgb;
    vec3 l = texture(u_Input, vec2(TexCoords.x - x, TexCoords.y - y)).rgb;
    vec3 m = texture(u_Input, vec2(TexCoords.x + x, TexCoords.y - y)).rgb;
